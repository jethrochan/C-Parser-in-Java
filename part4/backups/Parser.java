...
public class Parser {
    ...
    {
      int [] arr = new int [2];
      ArrayList<Symbols> id_list;// = new ArrayList<Symbols>();
      Symbols temp;

      while(!st.pls_dont_disappear.isEmpty())
      { 
	SymbolTable look = st.pls_dont_disappear.pop();

	if(!look.ids.isEmpty() && look.ids.peek().size() > 0)
	{
	  id_list = look.ids.pop();
	  for(int i = 0; i < id_list.size(); i++)
	  {
	    temp = id_list.get(i);
            System.err.println(temp.str); //var
	    ...
	    for(int j = 0; j < temp.assigned.size(); i++)
	    {
	      arr = temp.assigned.get(j);
	      System.err.print(" "+arr[0]);
	      if(arr[1] > 1)
	        System.err.print("("+arr[1]+")");
     	    }
	  
				
	    System.err.println("  used on: "
				+"used lines/times here");
	  }
	  look.ids.pop();
	}
      }
 	...
 	if(is(TK.ID))
	{
		use = false;
		assign = true;
		st.check(tok.string, tok.lineNumber, use, assign);
	    	mustbe(TK.ID);
